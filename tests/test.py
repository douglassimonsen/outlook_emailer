...
def test_access():
    outlook_emailer.get_exchangelib_account(access_account="mwhamilton6")
...
def test_emailer_skip():
    ...
    outlook_emailer.send_email(account_email="mwhamilton6", to_list="mwhamilton6", subject=subject)
    time.sleep(15)  # this is necessary because the server takes a second to log a sent email
    outlook_emailer.send_email(account_email="mwhamilton6", to_list="mwhamilton6", subject=subject, max_send_interval="daily")

    account = outlook_emailer.get_exchangelib_account(access_account="mwhamilton6")
